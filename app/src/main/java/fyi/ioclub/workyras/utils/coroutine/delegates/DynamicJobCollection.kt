...
class DynamicJobCollection(value: Collection<Job> = emptySet()) :
    ReadWriteProperty<Any?, Collection<Job>> {

    private var cache: Collection<Job> = value

    override operator fun getValue(thisRef: Any?, property: KProperty<*>) = cache

    override operator fun setValue(thisRef: Any?, property: KProperty<*>, value: Collection<Job>) {
        cache.forEach(Job::cancel)
        cache = value
    }
}