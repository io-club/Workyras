...
object StringResource : ToLateInit<Context> {

    override val toLateInit get() = !::_intFormat.isInitialized

    val intFormat get() = _intFormat
    private lateinit var _intFormat: String

    override fun lateInit(param: Context) {
        with(param) {
            _intFormat = getString(R.string.int_format)
        }
    }
}